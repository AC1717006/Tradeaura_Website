...
## CI/CD (since 2026-09-04)
Deployments run automatically via `.github/workflows/site-deploy.yml`:
push to main (or news/instagram bot completion) → build → verify-dist →
verify-routes → secret scan → sanity counts → OIDC (TradeAuraSiteDeploy) →
deletion guard → S3 sync → CloudFront invalidation + wait → live checks.
Manual tarball deploys from CloudShell are retired.
